...
## ✨ Características Principales (v1.9.0)
...
# Compilar para Producción (Genera Zenit_1.9.0_x64-setup.exe)
npm run tauri build
```
...
1. **`minimize_app`**: Minimiza el kiosko de forma segura y lanza la "Ventana de Retorno" (centrada verticalmente en el lateral derecho del monitor) para permitir la interacción libre con Windows.
2. **`restore_app`**: Oculta la ventana flotante de retorno y recupera el foco de la ventana principal de especificaciones.
3. **`set_always_on_top`**: Alterna la jerarquía de la ventana para asegurar que Zenit deba permanecer siempre al frente de la pantalla.
4. **`quit_app`**: Cierre administrativo que detiene de manera limpia todos los procesos y hooks.

### 🎥 Bóveda de Videos (`vault.rs`)

1. **`list_custom_videos`**: Escanea el directorio de recursos para identificar videos locales.
2. **`save_custom_video`**: Gestiona la importación de nuevos archivos de video a la bóveda local.
3. **`delete_custom_video`**: Elimina recursos de forma física y limpia la base de datos de alias.
